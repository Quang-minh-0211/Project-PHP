...
<head>
    ...
    <link rel="stylesheet" href="manager-game-style.css">
</head>
        ...
        <div class="content">
            <div class="hero__container_1">
                ...
                if (isset($_POST["add-game"])) {
                    ...
                    if (count($errors) > 0) {
                        ...
                        $sql = "INSERT INTO game (nameGame, money_unit, money) VALUES (?, ?, ?)";
                        $stmt = mysqli_stmt_init($conn);
                        $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
                        if ($prepareStmt) {
                            mysqli_stmt_bind_param($stmt, "ssi", $nameGame, $money_unit, $money);
                            mysqli_stmt_execute($stmt);
                            echo "<div class='alert alert-success'>Successfully</div>";
                            // thay đổi đường dẫn trong header
                            // header("Location: manager-game.php");
                            // exit();
                        } else {
                ...
                <form action="manager-game.php" method="post">
                    <ul>
                        <li>
                            <label id="label-1" for="nameGame">Tên</label>
                            <input type="text" name="nameGame" id="nameCall">
                        </li>
                        <li>
                            <label id="label-2" for="money-unit">Tên đơn vị tiền</label>
                            <input type="text" name="money_unit" id="money-unit">
                        </li>
                        <li>
                            <label id="label-3" for="money">Số tiền</label>
                            <input type="text" name="money" id="money">
                        </li>

                    </ul>
                    <div class="add-btn">
                        <button type="submit" name="add-game" class="add-button">Thêm</button>
                    </div>
                </form>
                <script>
            ...
            <div class="hero__container_2">
                <div class="hero2-title">
                    <p><b>Danh sách game</b></p>
                </div>
                <?php
                require_once "database.php";
                $sql_tabel = "SELECT * FROM game";
                ...
                <div class="tabel-container">
                    ...
                    while ($row = mysqli_fetch_assoc($result_tabel)) {
                        ...
                        echo "<td>" . $row['money_unit'] . "</td>";
                        echo "<td>" . $row['money'] . "</td>";
                        //đổi tên biến của chức xoá
                        echo "<td><button class='delete-button' onclick='deleteGame(" . $row['id'] . ")'>Delete</button></td>";
                        echo "</tr>";
                    }
                    echo "</tabel>";
                    // mysqli_close($conn);
                ...
                <script>
                    ...
                    function deleteGame(gameId) {
                        ...
                        if (confirmation) {
                            ...
                            // đổi tên biến id
                            xhr.open("GET", "delete-game.php?id=" + gameId, true);
                            xhr.send();
                        }
                    }
                </script>
